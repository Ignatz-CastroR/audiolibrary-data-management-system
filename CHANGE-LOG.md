...
- `ONE-TIME-CODE-FUNC.cbl`'s header claimed the recovery-code
  ...
  below, where 'O' was actually pulled from the alphabet instead of
...
- Removed `SET ENVIRONMENT "COB_SCREEN_EXCEPTIONS" TO "1"` from both
  ...
  therefore ruled out on purpose, not left unexplored; it would have been overkill.
  Left a comment in both files' `MAIN-PAR` explaining why, and pointing at the
